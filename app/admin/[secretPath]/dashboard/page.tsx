...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-black/60 px-4 py-3 backdrop-blur-md md:px-6">
        <h1 className="text-lg font-semibold text-[var(--logo-gold-yellow)]">
          VCEMUN Admin
        </h1>
        <Button
          variant="outline"
          size="sm"
          className="border-white/20 text-muted-foreground hover:bg-white/10"
      ...
      <main className="flex-1 p-4 md:p-6">
        <Tabs defaultValue="priority" className="w-full">
          <TabsList className="mb-4 h-12 w-full max-w-md border border-white/10 bg-black/40 p-1 backdrop-blur-sm md:max-w-sm">
            <TabsTrigger
              value="priority"
              className="flex-1 data-[state=active]:bg-[var(--logo-gold-yellow)] data-[state=active]:text-black"
            >
