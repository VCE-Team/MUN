...
export function DynamicRoundDetailView({
  id,
  onBack,
}: {
  id: string;
  ...
  useEffect(() => {
    ...
    fetch(
      `${appConfig.backendUrl}/api/admin/dynamic-round-registrations/${id}`,
      {
  ...
  const fetchScreenshot = () => {
    ...
    setScreenshotUrl(null);
    fetch(
      `${appConfig.backendUrl}/api/admin/dynamic-round-registrations/${id}/screenshot`,
      { headers: getAdminHeaders() }
    )
