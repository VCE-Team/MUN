...
import {
  DynamicRoundRegistrationSchema,
  dynamicRoundRegistrationSchema,
} from '@/schemas/dynamicRoundRegistrationForm';
...
/**
 * Dynamic Round Registration Form Component
 *
 * Handles multi-step registration for Dynamic Round participants:
 ...
 * - Other Colleges: ₹1500
 */
export function DynamicRoundRegistrationForm() {
  const [step, setStep] = useState(1);
  const [isLoading, setIsLoading] = useState(false);
  const [isReviewMode, setIsReviewMode] = useState(false);
  const router = useRouter();

  const form = useForm<DynamicRoundRegistrationSchema>({
    resolver: zodResolver(dynamicRoundRegistrationSchema),
    defaultValues: {
  ...
  // Dynamic Round pricing: inHouse = ₹800, otherColleges = ₹1500
  const registrationFee = targetAudience === 'inHouse' ? 800 : 1500;

  /**
   * Check if email already exists in Dynamic Round or past registrations
   */
  ...
  const validateStep = async (stepNumber: number): Promise<boolean> => {
    let fieldsToValidate: (keyof DynamicRoundRegistrationSchema)[] = [];
  ...
  /**
   * Submit Dynamic Round registration
   */
  async function onSubmit(values: DynamicRoundRegistrationSchema) {
    setIsLoading(true);
    try {
      ...
      const response = await fetch(
        `${appConfig.backendUrl}/api/dynamic-round-register`,
        {
          method: 'POST',
          headers: {
      ...
      if (data.success) {
        toast({
          ...
          description:
            'Dynamic Round registration completed! You will receive a confirmation email shortly.',
        });
        setTimeout(() => {
          router.push('/');
        }, 3000);
      ...
      console.error('Error during Dynamic Round registration:', error);
      let errorMessage =
  ...
  const getStepTitle = () => {
    switch (step) {
      ...
      default:
        return 'Dynamic Round Registration';
    }
  };
